...
const App: React.FC = () => {
  ...
  const handleSchedulePost = async (caption: string) => {
      ...
      try {
          ...
          if (!response.ok) {
              let errorMessage = 'Scheduling failed with an unknown error.';
              try {
                  const errorData = await response.json();
                  errorMessage = errorData.message || 'The server returned an error without a specific message.';
              } catch (jsonError) {
                  const textError = await response.text();
                  errorMessage = `The server returned an unexpected response. Details: ${textError.substring(0, 200)}...`;
              }
